...
public class ProfileExportDataBinding implements DataBinding {

    private static Log log = LogFactory.getLog(ProfileExportDataBinding.class);
    public static String FILENAME_SCHEMA_CHAIN_ROOT = "profile-export.xsd";

    // just classes in the same package where can find the above respective
    // schema files via Class.getResource*
    public static Class<?> RESOURCE_CLASS = ProfileExportDataBinding.class;

    private ValidatingUnmarshaler validatingUnmarshaler;

    public static Class<?>[] FACTORIES = { 
    	org.plasma.test.sdo.export.profile.ObjectFactory.class,
    };

    @SuppressWarnings("unused")
    private ProfileExportDataBinding() {
    }

    public ProfileExportDataBinding(BindingValidationEventHandler validationEventHandler)
