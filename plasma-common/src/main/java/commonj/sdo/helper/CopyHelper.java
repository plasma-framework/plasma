...
public interface CopyHelper {
   ...
   * for each property where property.getType().isDataType() is true. The value
   * of such a Property property in copiedDataObject is:
   * dataObject.get(property) for single-valued Properties
   * (copiedDataObject.get(property) equals() dataObject.get(property)), or a
   * List where each member is equal to the member at the same index in
   * dataObject for multi-valued Properties
   * copiedDataObject.getList(property).get(i) equals()
   * dataObject.getList(property).get(i) The copied Object is unset for each
   * Property where property.getType().isDataType() is false since they are not
   * copied. Read-only properties are copied. A copied object shares metadata
   * with the source object sourceDO.getType() == copiedDO.getType() If a
   * ChangeSummary is part of the source DataObject the copy has a new, empty
   * ChangeSummary. Logging state is the same as the source ChangeSummary.
   * 
   * @param dataObject
   *          to be copied
  ...
  /**
   * Create a deep copy of the DataObject tree: Copies the dataObject and all
   * its {@link commonj.sdo.Property#isContainment() contained} DataObjects
   ...
   * IllegalArgumentException is thrown. If a ChangeSummary is part of the copy
   * tree the new ChangeSummary refers to objects in the new DataObject tree.
