...
public enum SysDataType implements PlasmaEnum {
   ...
   * 
   * SERIAL is an alias for BIGINT UNSIGNED NOT NULL AUTO_INCREME NT UNIQUE.
   * 
   * Some things you should be aware of with respect to BIGINT co lumns:
   * 
   * All arithmetic is done using signed BIGINT or DOUBLE values , so you should
   * not use unsigned big integers larger than 9223372036854775807 (63 bits)
   ...
   * 
   * When using integers to store large unsigned values in a BIGI NT column.
   * 
   * In MIN(col_name) or MAX(col_name), where col_name refers to a BIGINT
   * column.
   * 
   * When using operators (+, -, *, and so on) where both operand s are
   * integers.
   * 
   * You can always store an exact integer value in a BIGINT colu mn by storing
   ...
   * 
   * The -, +, and * operators use BIGINT arithmetic when both op erands are
   ...
   * The maximum number of digits (M) for DECIMAL is 65. The maximum number of
   * supported decimals (D) is 30. If D is omitted, the default is 0. If M is
   * omitted, the default is 10. UNSIGNED, if specified, disallows negative
   * values. All basic calculations (+, -, *, /) with DECIMAL columns are done
  ...
  /**
   * These types are synonyms for DECIMAL. The FIXED synonym is available for
   ...
   * 
   * M is the total number of digits and D is the number of digit s following
  ...
  /**
   * A normal-size (double-precision) floating-point number. Permissible values
   ...
   * M is the total number of digits and D is the number of digit s following
   * the decimal point. If M and D are omitted, values are stored to the limits
   * permitted by the hardware. A double-precision floating-point number is
  ...
  /**
   * These types are synonyms for DOUBLE. Exception: If the REAL_AS_FLOAT SQL
   * mode is enabled, REAL is a synonym for FLOAT rather than DOUBLE.
   * <p>
   * </p>
   ...
   * DATE values in 'YYYY-MM-DD' format, but permits assignment of values to
   * DATE columns using either strings or numbers.
   * <p>
   * </p>
   ...
   * can also set any TIMESTAMP column to the current date and time by assigning
   * it a NULL value, unless it has been defined with the NULL attribute to
   * permit NULL values.
   * <p>
   * </p>
   ...
   * values. In four-digit format, values display as 1901 to 2155, and 0000. In
   * two- digit format, values display as 70 to 69, representing years from 1970
   * to 2069. MySQL displays YEAR values in YYYY or YYformat, but permits
   * assignment of values to YEAR columns using either strings or numbers.
   * <p>
   * </p>
   ...
   * rather than nonbinary character strings. M represents the column length in
   * bytes.
   * <p>
   * </p>
   ...
   * value is stored using a 1-byte length prefix that indicates the number of
   * bytes in the value.
   * <p>
   * </p>
   ...
   * characters. Each TINYTEXT value is stored using a 1-byte length prefix that
   ...
   * MEDIUMBLOB value is stored using a 3-byte length prefix that indicates the
   ...
   * effective maximum length is less if the value contains multi-byte
   * characters. Each MEDIUMTEXT value is stored using a 3-byte length prefix
   ...
   * memory. Each LONGBLOB value is stored using a 4-byte length prefix that
   * indicates the number of bytes in the value.
   * <p>
   * </p>
   ...
   * protocol and available memory. Each LONGTEXT value is stored using a 4-byte
   ...
   * value. ENUM values are represented internally as integers.
   * 
   * An ENUM column can have a maximum of 65,535 distinct element s. (The
   * practical limit is less than 3000.) A table can have no more than 255
   * unique element list definitions among its ENUM and SET columns considered
   * as a group. For more information on these limits,
   * <p>
   * </p>
   ...
   * must be chosen from the list of values 'value1', 'value2', ... SET values
   * are represented internally as integers.
   * 
   * A SET column can have a maximum of 64 distinct members. A ta ble can have
   * no more than 255 unique element list definitions among its ENUM and SET
