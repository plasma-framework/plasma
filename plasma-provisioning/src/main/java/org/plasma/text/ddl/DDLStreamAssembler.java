...
public class DDLStreamAssembler extends DDLAssembler implements StreamAssembler {
  ...
  private void createSchemas() throws IOException {
    for (Schema schema : this.schemas.getSchemas()) {
      ...
      if (tables.size() > 0) {
        String ddl = createSchema(schema);
    ...
    for (Schema schema : this.schemas.getSchemas()) {
      for (Table table : schema.getTables()) {
        for (Fk fk : table.getFks()) {
          String toSchemaName = fk.getToSchema();
          if (toSchemaName == null)
            toSchemaName = schema.getName();
          Table toTable = getTable(fk.getToTable(), toSchemaName);
  ...
  private Table getTable(String name, Schema schema) {
    return getTable(name, schema.getName());
  }

  private Table getTable(String name, String schemaName) {
