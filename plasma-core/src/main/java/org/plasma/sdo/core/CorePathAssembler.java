...
public class CorePathAssembler implements PathAssembler {
  ...
  public CorePathAssembler(DataObject pathTarget) {
    ...
    if (log.isDebugEnabled())
      log.debug("finding paths from root: " + root.getType().getName() + "("
    ...
    if (level == currentPath.size()) {
      ...
      if (currentPath.size() != level)
        throw new IllegalStateException("unexpected path size, " + currentPath.size()
            + " and traversal level, " + level);
      int siblingIndex = 0;
      // If current node is a sibling (e.g. same parent and same source
      // property), assume
      // it's index is the previous path-node index +1
      if (pathNode.getSource().equals(source)) {
        if (pathNode.getSourceProperty().getName().equals(sourceProperty.getName()))
          siblingIndex = pathNode.getIndex() + 1;
      }

      currentPath.push(new PathNode(target, siblingIndex, source, sourceProperty));
  ...
  private Stack<PathNode> getMinimumPath() {
    if (pathList.size() == 0) {
      DataObject root = this.pathTarget.getDataObject().getDataGraph().getRootObject();
      throw new IllegalStateException("no paths from root data-object, " + root.getType().getURI()
          + "#" + root.getType().getName() + " (" + ((PlasmaNode) root).getUUIDAsString() + ") "
          + "to target, " + this.pathTarget.getDataObject().getType().getURI() + "#"
          + this.pathTarget.getDataObject().getType().getName() + " ("
          + this.pathTarget.getUUIDAsString() + ")");
    }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (pathNode.getSource() != null) {
        ...
        if (pathNode.getSourceProperty().isMany())
          tokens.add(pathNode.getSourceProperty().getName() + PATH_INDEX_RIGHT
              + String.valueOf(pathNode.getIndex()) + PATH_INDEX_LEFT);
        else
