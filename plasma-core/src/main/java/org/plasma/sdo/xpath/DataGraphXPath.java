...
/**
 * An XPath implementation for a {@link commonj.sdo.DataGraph data graph} based
 * on Jaxen.
 *
 * <p>
 * This is the main entry point for matching an XPath against a
...
public class DataGraphXPath extends DefaultXPath {
   ...
   * The target Property is the result or end point Property after resolving an
   * XPATH.
   * 
   * @return an array with either 1 single result or multiple results.
   * @throws IllegalXPathArgumentException
   *           if no results are found or if multiple results are found but the
   ...
   *           after resolving an XPATH.
   * @throws JaxenException
   *           as required by the Jaxen API
   * @throws InvalidMultiplicityException
   *           if the given XPATH resulted in multiple values for a singular
   *           property
   */
  @SuppressWarnings("unchecked")
  public DataGraphNodeAdapter[] getResults(DataObject root) throws JaxenException,
    ...
    if (adapters.length > 1)
      for (DataGraphNodeAdapter adapter : adapters)
        if (!adapter.getSourceProperty().isMany())
          throw new InvalidMultiplicityException("given XPATH '" + this.toString()
              + "' resulted in multiple values for singular " + "property, "
              + adapter.getSource().getType().getURI() + "#"
   ...
   * 
   * @return an array of Property results or zero length array if no results are
   *         found.
   * @throws JaxenException
   *           as required by the Jaxen API
   ...
   */
  @SuppressWarnings("unchecked")
  public XPathDataProperty[] findProperties(DataObject root) throws JaxenException,
      ...
      if (!(node instanceof XPathDataProperty)) {
        if (node instanceof XPathDataObject)
          throw new InvalidEndpointException("expected Property results rather than "
              + "Data Object results for the given XPATH '" + this.toString() + "'");
   ...
   * target Property is not a "many" Property, i.e. a multi-valued property. The
   * target Property is the result or end point Property after resolving an
   * XPATH.
   * 
   * @return an array of Data Object results.
   * @throws IllegalXPathArgumentException
   *           if other than Data Object results are found or if multiple
   *           results are found but the target Property is not a "many"
   *           Property, i.e. a multi-valued Property. The target Property is
   *           the result or end point Property after resolving an XPATH.
   * @throws JaxenException
   *           as required by the Jaxen API
   * @throws InvalidEndpointException
   *           when the result or terminating node or nodes for an XPATH
   *           expression are not Data Object nodes
   * @throws InvalidMultiplicityException
   *           when multiple results are detected for a singular property
   */
  @SuppressWarnings("unchecked")
  public XPathDataObject[] findDataObjects(DataObject root) throws JaxenException,
    ...
    for (int i = 0; i < nodes.size(); i++) {
      ...
      if (nodes.size() > 1)
        if (!adapters[i].getSourceProperty().isMany())
          throw new InvalidMultiplicityException("given path '" + this.toString()
              + "' resulted in multiple values for singular " + "property, "
              + adapters[i].getSource().getType().getURI() + "#"
