...
class FumlRelationCache implements RelationCache {
   ...
   * 
   * @see
   * org.plasma.sdo.repository.fuml.RelationCache#isRelation(org.plasma.sdo.
   * repository.fuml.FumlClassifier,
   * org.plasma.sdo.repository.fuml.FumlClassifier,
   * org.plasma.sdo.AssociationPath)
   */
  @Override
  public boolean isRelation(Class_ target, Class_ source, AssociationPath relation) {
    ...
    case singular:
      ...
      if (result == null) {
        ...
        if (log.isDebugEnabled())
          log.debug("cache hit (" + result.isRelated() + ") lateral-singular relation: "
              + target.getName() + "/" + source.getName() + " (" + key + ")");
      }
  ...
  private boolean isLateralSingularRelation(Class_ targetType, Class_ sourceType,
      Property traversalSourceProperty, Map<String, Integer> visited) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("checking targetType  " + targetType.toString() + " with "
          + targetProperties.size() + " properties");
    }
    ...
    for (Property targetProperty : targetProperties) {
      ...
      if (targetProperty.getType().isDataType()) {
        if (log.isDebugEnabled())
          log.debug("not a reference property " + targetType.toString() + "."
              + getDebugName(targetProperty));
        continue; // not a reference
      }

      // We are traversing BACK-TO the source type, so need to look at the
      ...
      if (targetPropertyOpposite.isMany()) {
        if (log.isDebugEnabled())
          log.debug("opposite not singular " + targetType.toString() + "."
              + getDebugName(targetProperty));
        continue;
      }
      ...
      if (traversalSourceProperty != null) {
        ...
        if ((count = visited.get(linkKey)) == null)
          ...
          /*
           * org.modeldriven.fuml.repository.Property opposite =
           * targetProperty.getOpposite(); if (opposite != null &&
           * opposite.isSingular()) { log.warn("singular property " +
           * targetType.getName() + "." + (targetProperty.getName() == null ?
           * targetProperty.getXmiId() : targetProperty.getName()) +
           * " has opposite " + opposite.getClass_().getName()+"." +
           * (opposite.getName() == null ? opposite.getXmiId() :
           * opposite.getName()) + " which is also singular"); }
           */
          if (log.isDebugEnabled()) {
            log.debug("exiting - visited  " + targetType.toString() + "."
                + getDebugName(targetProperty));
          }
      ...
      if (log.isDebugEnabled()) {
        log.debug("comparing " + targetType.toString() + "." + getDebugName(targetProperty) + " ("
            + targetProperty.getType().getId() + ")" + "->" + sourceType.toString() + " ("
            + sourceType.getId() + ")");
      }
      ...
      if (targetProperty.getType().getId().equals(sourceType.getId())) {
        if (targetType.getId().equals(sourceType.getId())) {
          log.warn("potential circular reference: " + targetType.toString() + "."
              + getDebugName(targetProperty) + "->" + sourceType.toString());
        }
        if (log.isDebugEnabled())
        ...
        if (!(targetProperty.getType().getId().equals(targetType.getId()))) {
          ...
          if (log.isDebugEnabled())
            log.debug("traversing " + targetType.toString() + "." + getDebugName(targetProperty));
          // FIXME: classifiers returned via XMI ID have no properties !!
          // See re-lookup by qualified name below
          org.plasma.sdo.repository.Classifier tempClassifier = FumlRepository
              .getFumlRepositoryInstance().getClassifierById(targetProperty.getType().getId());

          String urn = tempClassifier.getArtifactNamespaceURI();
          String qualifiedName = urn + "#" + tempClassifier.getName();
          org.plasma.sdo.repository.Classifier targetClassifier = FumlRepository
              .getFumlRepositoryInstance().getClassifier(qualifiedName);
          ...
          if (isLateralSingularRelation(nextClassifier, sourceType, // Note:
                                                                    // stays
                                                                    // constant
                                                                    // across
                                                                    // traversals
              targetProperty, visited))
            return true;
        }
      }
    }
  ...
  /**
   * UML properties are not required to be named, so at the repository level for
   * debugging, construct some recognizable name.
   * 
   * @param prop
   *          the property
