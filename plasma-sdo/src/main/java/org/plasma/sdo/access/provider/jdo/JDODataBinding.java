...
public class JDODataBinding implements DataBinding {

    private static Log log = LogFactory.getLog(JDODataBinding.class);
    public static String FILENAME_SCHEMA_CHAIN_ROOT = "jdo.xsd";

    // just classes in the same package where can find the above respective
    // schema files via Class.getResource*
    public static Class<?> RESOURCE_CLASS = JDODataBinding.class;

    private static ValidatingUnmarshaler validatingUnmarshaler;

    public static Class<?>[] FACTORIES = { 
    	org.plasma.sdo.access.provider.jdo.ObjectFactory.class
    };

    @SuppressWarnings("unused")
    private JDODataBinding() {
    }

    public JDODataBinding(BindingValidationEventHandler validationEventHandler)
