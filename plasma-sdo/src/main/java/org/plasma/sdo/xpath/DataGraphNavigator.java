 ...
 * used by the Jaxen engine during evaluation. Use {@link DataGraphXPath}
 * to parse an XPath against a {@link commonj.sdo.DataGraph data graph}.
 * </p>
 *
 * @see XPath
...
public class DataGraphNavigator extends DefaultNavigator implements NamedAccessNavigator
{
	...
	public Iterator getChildAxisIterator(Object contextNode)
    {
        if ( contextNode instanceof XPathDataProperty )
        {
        	...
        	return this.getChildResult(
        			((XPathDataObject)contextNode).getDataObject());
        }
    }
	...
	private Iterator getChildResult(PlasmaDataObject source, String localName) {
    	...
    	if (sourceProp.getType().isDataType()) { // abort traversal       		
    		return new SingleObjectIterator(
    			new XPathDataProperty(sourceProp, source, sourceProp)); 
    	}
    	...
    	if (target != null) {
    		if (target instanceof List) {
    			...
    			return new SingleObjectIterator(
    				new XPathDataObject((PlasmaDataObject)target, 
    						source, sourceProp)); 
    	}    	
        return JaxenConstants.EMPTY_ITERATOR;
    }
	...
	private Iterator getChildResult(PlasmaDataObject source)
    {
    	...
    	PlasmaDataGraphVisitor visitor = new PlasmaDataGraphVisitor() {
			public void visit(DataObject target, DataObject traversalSource,
	...
	public Iterator getAttributeAxisIterator(
        ...
        if ( contextNode instanceof XPathDataObject ) {
        	...
        	return new SingleObjectIterator(new XPathDataProperty(prop,
            		adapter.getDataObject(), prop));
        }
        return JaxenConstants.EMPTY_ITERATOR;
    }
