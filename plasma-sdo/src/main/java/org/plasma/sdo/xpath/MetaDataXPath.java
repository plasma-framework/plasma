 ...
 * {@link commonj.sdo.Type} structures linked by {@link commonj.sdo.Property
 * properties}.
 * </p>
 * <p>
 ...
 * {@link commonj.sdo.Type} as input and therefore assumes a given XPath
 ...
 * {@link DataGraphXPath} to process mixed data and meta data XPath expressions.
 * </p>
 */
public class MetaDataXPath extends DefaultXPath {
  ...
  public MetaDataNodeAdapter[] getResults(Type root) throws JaxenException,
      ...
      for (MetaDataNodeAdapter adapter : adapters)
        if (!adapter.getSourceProperty().isMany())
          throw new InvalidMultiplicityException("given XPATH '" + this.toString()
              ...
              + adapter.getSource().getURI() + "#" + adapter.getSource().getName() + "."
   ...
   * @throws InvalidEndpointException
   *           when the result or terminating node or nodes are not Property
   *           nodes or if no result is found.
   */
