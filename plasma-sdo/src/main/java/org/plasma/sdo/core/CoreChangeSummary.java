...
public class CoreChangeSummary implements PlasmaChangeSummary, Serializable {
  ...
  public void deleted(DataObject dataObject) {
    ...
    if (dataObjectSettings == null) {
      ...
      // were changed
      if (ChangeType.MODIFIED.ordinal() == dataObjectSettings.getChangeType().ordinal()) {
        ...
        for (Setting setting : settings)
          if (setting.getProperty().getType().isDataType())
            throw new IllegalArgumentException("found unexpected modified property '"
                + setting.getProperty().getName() + "' for delete operation on data-object ("
                + dataNode.getUUIDAsString() + ") of type " + dataObject.getType().getURI() + "#"
        ...
        throw new IllegalArgumentException("found unexpected "
            + dataObjectSettings.getChangeType().toString() + " data-object (" + hashKey
            + ") of type " + dataObject.getType().getURI() + "#" + dataObject.getType().getName());
    }
  }
  ...
  /**
   * Clears the List of {@link #getChangedDataObjects changes} and turns change
   * logging on. No operation occurs if logging is already on.
   * 
   * @see #endLogging
   * @see #isLogging
   */
   ...
   * UnsupportedOperationException. Turns change logging off. No operation
   * occurs if logging is already off.
   * 
   * @see #beginLogging
   * @see #isLogging
   */
   ...
   * {@link #isDeleted deleted} objects in the List are references to objects at
   * the time that event logging was enabled;
   * <p>
   * Each changed object must have exactly one of the following methods return
   * true: {@link #isCreated isCreated}, {@link #isDeleted isDeleted}, or
  ...
  /**
   * Returns the {@link DataGraph data graph} associated with this change
   * summary or null.
   * 
  ...
  /**
   * Returns the value of the {@link DataObject#getContainer container} data
   * object at the point when logging {@link #beginLogging() began}.
   * 
   * @param dataObject
  ...
  /**
   * Returns the value of the {@link DataObject#getContainmentProperty
   * containment property} data object property at the point when logging
   * {@link #beginLogging() began}.
   * 
   * @param dataObject
   *          the object in question.
  ...
  /**
   * Returns the value of the {@link DataObject#getSequence sequence} for the
   * data object at the point when logging {@link #beginLogging() began}.
   * 
   * @param dataObject
   *          the object in question.
   * @return the old containment property.
   ...
   * representing the property value of the given <code>dataObject</code> at the
   * point when logging {@link #beginLogging() began}.
   * <p>
   * Returns null if the property was not modified and has not been
   * {@link #isDeleted(DataObject) deleted}.
   * 
   * @param dataObject
   *          the object in question.
   * @param property
  ...
  public Setting getOldValue(DataObject dataObject, Property property) {
    ...
    List<ChangeSummary.Setting> propertySettings = dataObjectSettings.getSettings(property
        .getName());
  ...
  /**
   * Returns a list of {@link ChangeSummary.Setting settings} that represent the
   * property values of the given <code>dataObject</code> at the point when
   * logging {@link #beginLogging() began}.
   * <p>
   * In the case of a {@link #isDeleted(DataObject) deleted} object, the List
   * will include settings for all the Properties.
   * <p>
   * An old value setting indicates the value at the point logging begins. A
   * setting is only produced for {@link #isModified modified} objects if either
   ...
   * are tracked. When a DataGraph is used, this is the same as
   * getDataGraph().getRootObject().
   * 
   ...
   * {@link #isLogging logging}. Any object that was added to the scope but was
   * not in the scope when logging began, will be considered created.
   * 
   * @param dataObject
   ...
   * {@link #isLogging logging}. An object that was contained in the scope when
   ...
   * {@link #isLogging logging}. Any object that is not in scope but was in
   * scope when logging began will be considered deleted.
   * 
   * @param dataObject
   *          the data object in question.
  ...
  public int getPathDepth(DataObject dataObject) {
    ...
    Change info = changedDataObjects.get(dataNode.getUUID());
    if (info == null)
      throw new IllegalArgumentException("given data-object (" + dataNode.getUUIDAsString()
          + ") not registed in change summary");
    return info.getPathDepthFromRoot();
  }

  /**
   * Indicates whether change logging is on (<code>true</code>) or off (
   * <code>false</code>).
   * 
  ...
  /**
   * This method is intended for use by service implementations only. Undoes all
   * changes in the log to restore the tree of DataObjects to its original state
   * when logging began. isLogging() is unchanged. The log is cleared.
   * 
   * @see #beginLogging
   * @see #endLogging
   * @see #isLogging
  ...
  public String toString() {
    ...
    for (DataObject dataObject : changedObjects) {
      ...
      result.append("\n\t<" + info.getChangeType().toString().toLowerCase() + " type=\""
          + dataObject.getType().getName() + "\"" + " path=\"" + info.getPathFromRoot() + "\""
      ...
      if (settings != null) {
        for (Setting setting : settings) {
          ...
          result.append(" timestamp=\""
              + String.valueOf(((PlasmaSetting) setting).getCreatedMillis()) + "\"");
     ...
     * <changeSummary create="E0004" delete="E0002"> <company
     * sdo:ref="#/company:company[1]" name="ACME" employeeOfTheMonth="E0002"/>
     * <departments sdo:ref="#/company:company[1]/departments[1]"> <employees
     * sdo:ref="E0001"/> <employees name="Mary Smith" SN="E0002"
     * manager="true"/> <employees sdo:ref="E0003"/> </departments>
     * </changeSummary> <company:company name="MegaCorp"
     * employeeOfTheMonth="E0004"> <departments name="Advanced Technologies"
     * location="NY" number="123"> <employees name="John Jones" SN="E0001"/>
     * <employees name="Jane Doe" SN="E0003"/> <employees name="Al Smith"
     * SN="E0004" manager="true"/> </departments> </company:company>
     * </sdo:datagraph>
     */
