...
public abstract class DefaultFactory {
   ...
   *          the SDO datatype
   * @param primitives
   *          whether to return a primitive type if possible based on the given
   *          SDO datatype
   * @return the 3Gl language specific type class name.
   */
  public java.lang.Class<?> getTypeClass(DataType dataType, boolean primitives) {
  ...
  /**
   * Collects class names from reference fields within this class and its
   * superclass ancestry.
   * 
   * @param pkg
   *          the package
  ...
  // FIXME: move to provisioning tool/helper/whatever
  @Deprecated
  protected void collectProvisioningSuperclasses(Package pkg, Class clss,
  ...
  // FIXME: move to provisioning tool/helper/whatever
  @Deprecated
  protected void collectProvisioningFields(Package targetPkg, Class targetClss, Package pkg,
      Class clss, Map<String, FieldAdapter> fields) {
    ...
    for (Property field : clss.getProperties()) {
      ...
      if (existing != null) {
        if (existing.getFieldClass().getName().equals(clss.getName())
          ...
          log.warn("Classifier " + targetClss.getUri() + "#" + targetClss.getName()
              + " inherits multiple identical properties '" + existing.getFieldClass().getUri()
              + "#" + existing.getFieldClass().getName() + "." + existing.getField().getName()
              + "'");
          continue;
        } else if (targetClss.getName().equals(clss.getName())
            && targetClss.getUri().equals(clss.getUri())) {
          throw new PropertyNameCollisionException("Classifier " + clss.getUri() + "#"
              + clss.getName() + " with property '" + field.getName()
              + "' inherits an identically named property from classifier "
              + existing.getFieldClass().getUri() + "#" + existing.getFieldClass().getName());
        } else {
          throw new PropertyNameCollisionException("Classifier " + targetClss.getUri() + "#"
              + targetClss.getName() + " inherits multiple properties named '" + field.getName()
              + "' from 2 parent classifiers, " + existing.getFieldClass().getUri() + "#"
              + existing.getFieldClass().getName() + " and " + clss.getUri() + "#" + clss.getName());
        }
      }
  ...
  protected MetaClassInfo getTypeClassName(TypeRef type) {
    ...
    if (type instanceof DataTypeRef) {
      ...
      result = new MetaClassInfo(dataTypeRef, sdoType, primitiveTypeClass, wrapperTypeClass,
  ...
  protected void createUnsetterDeclaration(Package pkg, Class clss, Property field,
    ...
    String definition = this.getWrappedDocmentations(field.getDocumentations(), 1);
    if (definition != null && definition.length() > 0) {
      addPropertyModelDocLinks(clss, field, typeClassName, buf);
    }
  ...
  private void addPropertyModelDocLinks(Class clss, Property field, MetaClassInfo typeClassName,
      TextBuilder buf) {
    buf.appendln(1, " * <p></p>");
    buf.appendln(1, " * <b>Property Definition: </b>");
  ...
  protected void createCreatorByAbstractClassDeclaration(Package pkg, Class clss, Property field,
    ...
    buf.appendln(1, "/**"); // begin javadoc
    buf.appendln(1,
        " * Creates and returns a new instance of the given subclass Type for abstract base Type {@link ");
    buf.append(typeClassName.getSimpleName());
    buf.append("} automatically establishing a containment relationship ");
    buf.append("through the object's reference property, <b>");
    buf.append(field.getName());
    ...
    // return
    buf.appendln(1,
        " * Returns a new instance of the given subclass Type for abstract base Type {@link ");
    buf.append(typeClassName.getSimpleName());
    buf.append("}.");
  ...
  protected void collectReferenceClassNames(Package pkg, Class clss, Map<String, String> nameMap,
      ClassNameResolver resolver) {
    collectReferenceClassNames(pkg, clss, nameMap, resolver, true, -1);
  }
   ...
   *          the Class
   * @param nameMap
   *          the name map used to store collected results
   * @param resolver
   *          the qualified name resolver
   */
  protected void collectReferenceClassNames(Package pkg, Class clss, Map<String, String> nameMap,
      ClassNameResolver resolver, boolean collectAbstractClasses, int maxLevel) {
  ...
  protected void collectDataClassNames(Package pkg, Class clss, Map<String, String> nameMap,
      ClassNameResolver resolver) {
    collectDataClassNames(pkg, clss, nameMap, resolver, true, -1);
  }
  ...
  protected void collectSuperClassNames(Package pkg, Class clss, Map<String, String> nameMap,
      ClassNameResolver resolver) {
    collectSuperClassNames(pkg, clss, nameMap, resolver, true, -1);
  }
    ...
    if (clss.getSuperClasses() != null && clss.getSuperClasses().size() > 0) {
      for (ClassRef cref : clss.getSuperClasses()) {
        ...
        collectSuperClassNames(superClassPackage, superClass, nameMap, resolver,
            collectAbstractClasses, maxLevel);
      }
    }
  }
  ...
  protected void collectReferenceFieldClassNamesDeep(Package pkg, Class clss,
      Map<String, String> nameMap, ClassNameResolver resolver, boolean collectAbstractClasses,
      int maxLevel) {
    ...
    if (clss.getSuperClasses() != null && clss.getSuperClasses().size() > 0) {
      for (ClassRef cref : clss.getSuperClasses()) {
        ...
        Package superClassPackage = this.context.findPackage(cref);
        // recurse
        collectReferenceFieldClassNamesDeep(superClassPackage, superClass, nameMap, resolver,
            collectAbstractClasses, maxLevel);
      }
    }
  }
  ...
  protected void collectDataFieldClassNamesDeep(Package pkg, Class clss,
    ...
    if (clss.getSuperClasses() != null && clss.getSuperClasses().size() > 0) {
      for (ClassRef cref : clss.getSuperClasses()) {
        ...
        if (superClass.isAbstract() && !collectAbstractClasses)
          continue;
        // Note: don't map the superclass qualified name - we are collecting
        // data field names hers
        Package superClassPackage = this.context.findPackage(cref);
        // recurse
        collectDataFieldClassNamesDeep(superClassPackage, superClass, nameMap, resolver,
            collectAbstractClasses, maxLevel);
      }
    }
  }
  ...
  protected Map<String, String> createFieldImportMap(Package pkg, Class clss,
    ...
    collectDataFieldClassNamesDeep(pkg, clss, nameMap, resolver, true, -1);
