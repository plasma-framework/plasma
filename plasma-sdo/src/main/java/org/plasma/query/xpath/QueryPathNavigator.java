...
/**
 * This class supports navigation of a PlasmaQuery path by the Jaxen XPATH
 * engine.
 *
 * <p>
...
public class QueryPathNavigator extends DefaultNavigator implements NamedAccessNavigator {
  ...
  public Iterator getChildAxisIterator(Object contextNode) {
    if (contextNode instanceof XPathProperty) {
      ...
      return new SingleObjectIterator(new XPathProperty(property, property.getType())); // FIXME
    } else
  ...
  private Iterator getChildResult(PlasmaType source, String localName) {
    ...
    if (prop.getType().isDataType()) { // abort traversal
      return new SingleObjectIterator(new XPathProperty(prop, source));
    }
    ...
    return new SingleObjectIterator(new XPathType(prop.getType(), source, prop));
  ...
  public Iterator getAttributeAxisIterator(Object contextNode, String localName,
    ...
    if (contextNode instanceof XPathType) {
      ...
      return new SingleObjectIterator(new XPathProperty(prop, null, prop)); // FIXME
    }
    return JaxenConstants.EMPTY_ITERATOR;
  }
