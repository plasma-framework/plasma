...
public class AnnotationMetamodelAssembler implements AnnotationConverter {
  ...
  public Model buildModel() {
    ...
    for (java.lang.Class<?> c : dataObjectClasses) {
      ...
      Class clss = this.createClass(type.name(), type.isAbstract(), c, type.superTypes(), packg);
  ...
  private Package createPackage(String nameToken, Namespace namespace,
    ...
    if (leafPackage) {
      ...
      org.plasma.config.annotation.NamespaceProvisioning nsProvAnnot = javaPackage
          .getAnnotation(org.plasma.config.annotation.NamespaceProvisioning.class);
      if (nsProvAnnot != null) {
    ...
    if (leafPackage) {
      ...
      if (alias == null) {
        ...
        if (alias.getLocalName() != null && alias.getLocalName().length() > 0)
          log.warn("alias local name for package should not be used in this context - overwriting local name for "
              + javaPackage.getName());
