 ...
 * >Enumeration</a> represents the domain model enumeration <b>SysDataType</b>
...
public enum SysDataType implements PlasmaEnum {
   ...
   * 
   * When you insert or update a row in the table, the value for the CHAR column
   * has the fixed length.
   * 
   * If you give a shorter value, then the value is blank-padded to the fixed
   * length.
   * 
   * If a value is too large, Oracle Database returns an error.
   * 
   * Oracle Database compares CHAR values using blank-padded comp arison
   * semantics.
   * <p>
   * </p>
  ...
  /**
   * The VARCHAR2 datatype stores variable-length character strings. When you
   * create a table with a VARCHAR2 column, you specify a maximum string length
   * (in bytes or characters) between 1 and 4000 bytes for the VARCHAR2 column.
   * For each row, Oracle Database stores each value in the column as a
   * variable-length field unless a value exceeds the column's maximum length,
   * in which case Oracle Database returns an error. Using VARCHAR2 and VARCHAR
   * saves on space used by the table.
   * 
   * For example, assume you declare a column VARCHAR2 with a max imum size of
   * 50 characters. In a single-byte character set, if only 10 characters are
   ...
   * 
   * Oracle Database compares VARCHAR2 values using nonpadded com parison
   * semantics.
   * <p>
   * </p>
  ...
  /**
   * The NCHAR datatype stores fixed-length character strings that correspond to
   * the national character set. The maximum length of an NCHAR column is 2000
   * bytes. It can hold up to 2000 characters. The actual data is subject to the
   * maximum byte limit of 2000. The two size constraints must be satisfied
  ...
  /**
   * The NVARCHAR2 datatype stores variable length character strings. The
   * maximum length of an NVARCHAR2 column is 4000 bytes. It can hold up to 4000
   * characters. The actual data is subject to the maximum byte limit of 4000.
   ...
   * 
   * LONG datatype columns are used in the data dictionary to sto re the text of
   * view definitions. You can use LONG columns in SELECT lists, SET clauses of
   * UPDATE statements, and VALUES clauses of INSERT statements.
   * <p>
   * </p>
   ...
   * datatype. Each BINARY_DOUBLE value requires 9 bytes, including a length
   * byte.
   * <p>
   * </p>
   ...
   * 
   * Oracle Database can store dates in the Julian era, ranging f rom January 1,
   * 4712 BCE through December 31, 9999 CE (Common Era, or 'AD'). Unless BCE
   * ('BC' in the format mask) is specifically used, CE date entries are the
   * default.
   * 
   ...
   * 
   * BLOBs participate fully in transactions. Changes made to a B LOB value by
   * the DBMS_LOB package, PL/SQL, or the OCI can be committed or rolled back.
  ...
  /**
   * The CLOB and NCLOB datatypes store up to 128 terabytes of character data in
   * the database. CLOBs store database character set data, and NCLOBs store
   * Unicode national character set data. Storing varying-width LOB data in a
   * fixed-width Unicode character set internally enables Oracle Database to
   ...
   * files outside the database. A BFILE column or attribute stores a file
   * locator that points to an external file containing the data. The amount of
   * BFILE data that can be stored is limited by the operating system.
   * 
   * BFILEs are read only; you cannot modify them. They support o nly random
   * (not sequential) reads, and they do not participate in transactions. The
   * underlying operating system must maintain the file integrity, security, and
   * durability for BFILEs. The database administrator must ensure that the file
   * exists and that Oracle Database processes have operating-system read
  ...
  /**
   * RAW is a variable-length datatype like the VARCHAR2 character datatype,
   * except Oracle Net Services (which connects user sessions to the instance)
   * and the Import and Export utilities do not perform character conversion
   * when transmitting RAW or LONG RAW data. In contrast, Oracle Net Services
   * and Import/Export automatically convert CHAR, VARCHAR2, and LONG data
   * between the database character set and the user session character set, if
  ...
  /**
   * Oracle Database uses a ROWID datatype to store the address (rowid) of every
   * row in the database.
   * 
   * Physical rowids store the addresses of rows in ordinary tabl es (excluding
   * index-organized tables), clustered tables, table partitions and
   * subpartitions, indexes, and index partitions and subpartitions.
   * 
   * Logical rowids store the addresses of rows in index-organize d tables.
   * <p>
   * </p>
