	    ...
	    <xsl:variable name="extends">  
		    <xsl:choose>	                       
		        <xsl:when test="count(generalization) = 1">
                    ...
                    <xsl:variable name="generalizationClassName" select="/uml:Profile//packagedElement[@xmi:id=$general]/@name"/>
...
<xsl:template name="emitImportsForOwnedAttribute">
    ...
    <xsl:when test="$typeId != ''">
          <xsl:variable name="classType" select="/uml:Profile//packagedElement[@xmi:id=$typeId]/@xmi:type"/>
         <xsl:choose>
            ...
            <xsl:when test="$classType = 'uml:Stereotype'">
                <xsl:variable name="umlTypeHref" select="/uml:Profile//packagedElement[@xmi:id=$typeId]/ownedAttribute/type[@xmi:type='uml:Class']/@href"/>
                <xsl:variable name="typeName" select="substring-after($umlTypeHref, '#')"/>
                <xsl:variable name="javaTypeName">
                    <xsl:call-template name="getJavaTypeForUMLType">
                        <xsl:with-param name="type" select="$typeName" />
            ...
            <xsl:otherwise>
                <xsl:variable name="className" select="/uml:Profile//packagedElement[@xmi:id=$typeId]/@name"/>
